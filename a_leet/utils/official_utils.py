...
class ProblemData:
    ...
    def get_list_data(custom_data) -> list:
        ...
        for prob in custom_data:
            year, exam, subject = prob.leet.year, prob.leet.exam, prob.subject
            key = f'{year}{exam[0]}{subject[0]}'
            if key not in organized_dict:
                organized_dict[key] = []
            problem_url = reverse('leet:problem-detail', args=[prob.id])
            list_item = {
...
class AdminListData:
    ...
    def __post_init__(self):
        ...
        self.view_type = request_data.view_type
        self.page_number = request_data.page_number
        self.filterset = filters.LeetFilter(data=self.request.GET, request=self.request)

    def get_leet_context(self):
        leet_context = get_paginator_context(self.filterset.qs, self.page_number)
        for leet in leet_context['page_obj']:
            leet.updated_problem_count = sum(1 for prob in leet.problems.all() if prob.question and prob.data)
            leet.image_problem_count = sum(1 for prob in leet.problems.all() if prob.has_image)
        return leet_context
...
class AdminUpdateData:
    ...
    def process_post_request(self):
        ...
        for index, row in df.iterrows():
            problem = models.Problem.objects.get(leet=self.leet, subject=row['subject'], number=row['number'])
