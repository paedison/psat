...
from a_leet.utils.common_utils import RequestContext
...
class ViewConfiguration:
    ...
    url_admin = reverse_lazy('admin:a_leet_problem_changelist')
    url_list = reverse_lazy('leet:problem-list')
...
def official_problem_list_view(request: HtmxHttpRequest):
    ...
    context = update_context_data(
        ...
        problem_context=list_context.get_problem_context(filterset.qs)
    )

    if request_context.view_type == 'problem_list':
        return render(request, 'a_leet/problem_list_content.html', context)

    context = update_context_data(context, form=filterset.form, collections=list_context.get_collections())
    return render(request, 'a_leet/problem_list.html', context)
...
def official_problem_detail_view(request: HtmxHttpRequest, pk: int):
    ...
    config.url_admin = reverse_lazy(f'admin:a_leet_problem_change', args=[pk])
    ...
    template_nav = 'a_leet/snippets/navigation_container.html'
    ...
    if view_type == 'image':
        return render(request, 'a_leet/problem_detail.html#modal_image', context)  # noqa
    ...
    # pagination_url = reverse_lazy('leet:comment-problem')

    context = update_context_data(
        context,
        # icons
        icon_menu=icon_set_new.ICON_MENU['leet'],
    ...
    response = render(request, 'a_leet/problem_detail.html', context)
...
def official_rate_problem(request: HtmxHttpRequest, pk: int):
    ...
    return render(request, 'a_leet/snippets/rate_modal.html', context)
...
def official_solve_problem(request: HtmxHttpRequest, pk: int):
    ...
    return render(request, 'a_leet/snippets/solve_modal.html', context)
...
def official_memo_problem(request: HtmxHttpRequest, pk: int):
    ...
    if update_data.view_type == 'create' and request.method == 'POST':
        ...
        if create_form.is_valid():
            ...
            return render(request, 'a_leet/snippets/memo_container.html', context)
    ...
    if update_data.view_type == 'update':
        if request.method == 'POST':
            ...
            return render(request, 'a_leet/snippets/memo_container.html#update_form', context)  # noqa
    ...
    if update_data.view_type == 'delete' and request.method == 'POST':
        ...
        latest_record.save()
        memo_url = reverse_lazy('leet:memo-problem', args=[pk])
...
def official_collection_list_view(request: HtmxHttpRequest):
    ...
    return render(request, 'a_leet/collection_list.html', context)
...
def official_collection_create(request: HtmxHttpRequest):
    ...
    if view_type == 'create':
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect('leet:collection-list')
        else:
            form = forms.ProblemCollectionForm()
            context = update_context_data(form=form, url=reverse_lazy('leet:collection-create'), header='create')
            return render(request, 'a_leet/snippets/collection_create.html', context)
    ...
    if view_type == 'create_in_modal':
        if request.method == 'POST':
            ...
            if form.is_valid():
                create_new_collection(request, form)
                return redirect('leet:collect-problem', pk=problem_id)
        else:
            problem_id = request.GET.get('problem_id')
            form = forms.ProblemCollectionForm()
            context = update_context_data(
                form=form, url=reverse_lazy('leet:collection-create'),
...
def official_collection_detail_view(request: HtmxHttpRequest, pk: int):
    ...
    if view_type == 'update':
        if request.method == 'POST':
            ...
            context = update_context_data(
                form=form, url=reverse_lazy('leet:collection-detail', args=[pk]), header='update')
            return render(request, 'a_leet/snippets/collection_create.html', context)
    ...
    return render(request, 'a_leet/snippets/collection_item_card.html', context)
...
def official_collect_problem(request: HtmxHttpRequest, pk: int):
    if request.method == 'POST':
        ...
        return render(request, 'a_leet/snippets/collection_modal.html', context)
...
#     pagination_url = reverse_lazy('leet:comment-list')
#     context = update_context_data(
...
#         icon_question=icon_set_new.ICON_QUESTION,
#     )
#     return render(request, 'a_leet/comment_list.html', context)
#
#
...
#             return render(request, 'a_leet/snippets/comment.html', context)  # noqa
#
#
