...
from a_leet.models import choices
from a_leet.models.queryset import official_queryset as queryset
from common.constants import icon_set_new
from common.models import User


class Leet(models.Model):
    objects = queryset.LeetQuerySet.as_manager()
    year = models.PositiveSmallIntegerField(choices=choices.year_choice, default=datetime.now().year + 1, verbose_name='연도')
    exam = models.CharField(max_length=2, choices=choices.exam_choice, default='리트', verbose_name='시험')
    is_active = models.BooleanField(default=False, verbose_name='활성')

    class Meta:
        verbose_name = verbose_name_plural = "[기출문제] 00_LEET"
        ordering = ['-year']
        constraints = [
            models.UniqueConstraint(fields=['year', 'exam'], name='unique_leet_official_leet'),
        ]
    ...
    def get_admin_list_url():
        return reverse_lazy('leet:admin-official-list')

    def get_admin_official_detail_url(self):
        return reverse_lazy('leet:admin-official-detail', args=[self.id])

    def get_admin_leet_active_url(self):
        return reverse_lazy('leet:admin-official-leet-active', args=[self.id])

    def get_admin_predict_detail_url(self):
        return reverse_lazy('leet:admin-predict-detail', args=[self.id])

    def get_admin_predict_update_url(self):
        return reverse_lazy('leet:admin-predict-update', args=[self.id])

    def get_admin_predict_statistics_print_url(self):
        return reverse_lazy('leet:admin-predict-statistics-print', args=[self.id])

    def get_admin_predict_catalog_print_url(self):
        return reverse_lazy('leet:admin-predict-catalog-print', args=[self.id])

    def get_admin_predict_answer_print_url(self):
        return reverse_lazy('leet:admin-predict-answer-print', args=[self.id])

    def get_admin_predict_statistics_excel_url(self):
        return reverse_lazy('leet:admin-predict-statistics-excel', args=[self.id])

    def get_admin_predict_prime_id_excel_url(self):
        return reverse_lazy('leet:admin-predict-prime_id-excel', args=[self.id])

    def get_admin_predict_catalog_excel_url(self):
        return reverse_lazy('leet:admin-predict-catalog-excel', args=[self.id])

    def get_admin_predict_answer_excel_url(self):
        return reverse_lazy('leet:admin-predict-answer-excel', args=[self.id])

    @staticmethod
    def get_predict_list_url():
        return reverse_lazy('leet:predict-list')

    def get_predict_detail_url(self):
        return reverse_lazy('leet:predict-detail', args=[self.id])

    @staticmethod
    def get_predict_register_url():
        return reverse_lazy('leet:predict-register')

    def get_predict_answer_input_url(self, subject_field):
        return reverse_lazy('leet:predict-answer-input', args=[self.id, subject_field])

    def get_predict_answer_confirm_url(self, subject_field):
        return reverse_lazy('leet:predict-answer-confirm', args=[self.id, subject_field])

    def get_predict_modal_url(self):
        return reverse_lazy('leet:predict-modal', args=[self.id])
...
class ProblemTaggedItem(TaggedItemBase):
    ...
    user = models.ForeignKey(User, related_name='leet_tagged_items', on_delete=models.CASCADE)
...
class Problem(models.Model):
    ...
    like_users = models.ManyToManyField(User, related_name='liked_leet_problems', through='ProblemLike')
    rate_users = models.ManyToManyField(User, related_name='rated_leet_problems', through='ProblemRate')
    solve_users = models.ManyToManyField(User, related_name='solved_leet_problems', through='ProblemSolve')
    memo_users = models.ManyToManyField(User, related_name='memoed_leet_problems', through='ProblemMemo')
    comment_users = models.ManyToManyField(User, related_name='commented_leet_problems', through='ProblemComment')
    collections = models.ManyToManyField(
        'ProblemCollection', related_name='collected_leet_problems', through='ProblemCollectionItem')

    class Meta:
        verbose_name = verbose_name_plural = "[기출문제] 01_문제"
        ordering = ['leet', 'id']
        constraints = [
            models.UniqueConstraint(fields=['leet', 'subject', 'number'], name='unique_leet_official_problem'),
        ]
    ...
    @property
    def img_name(self):
        return f'LEET{self.year_ex_sub}{self.number:02}'

    @property
    def static_img_path(self):
        return f'image/LEET/{self.leet.year}/{self.img_name}.png'
    ...
    def get_absolute_url(self):
        return reverse_lazy('leet:problem-detail', args=[self.id])

    def get_like_url(self):
        return reverse_lazy('leet:like-problem', args=[self.id])

    def get_rate_url(self):
        return reverse_lazy('leet:rate-problem', args=[self.id])

    def get_solve_url(self):
        return reverse_lazy('leet:solve-problem', args=[self.id])

    def get_tag_url(self):
        return reverse_lazy('leet:tag-problem', args=[self.id])

    def get_collect_url(self):
        return reverse_lazy('leet:collect-problem', args=[self.id])

    def get_annotate_url(self):
        return reverse_lazy('leet:annotate-problem', args=[self.id])

    def get_comment_create_url(self):
        return reverse_lazy('leet:comment-problem-create', args=[self.id])

    def get_admin_change_url(self):
        return reverse_lazy('admin:a_leet_problem_change', args=[self.id])
...
class ProblemLike(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_likes')
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        if self.is_liked:
            return f'{self.problem.reference}(Liked)'
        return f'{self.problem.reference}(Unliked)'
...
class ProblemRate(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_rates')
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f'{self.problem.reference}({self.rating})'
...
class ProblemSolve(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_solves')
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        if self.is_correct:
            return f'{self.problem.reference}(Correct)-{self.user.username}'
        return f'{self.problem.reference}(Wrong)-{self.user.username}'
...
class ProblemMemo(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_memos')
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f'{self.problem.reference}-{self.user.username}'
    ...
    def get_memo_url(self):
        return reverse_lazy('leet:memo-problem', args=[self.problem_id])


class ProblemCollection(models.Model):
    objects = queryset.ProblemCollectionQuerySet.as_manager()
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_collections')
...
class ProblemAnnotation(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_annotations')
...
class ProblemComment(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_comments')
    content = models.TextField()
    like_users = models.ManyToManyField(
        User, related_name='liked_leet_problem_comments', through='ProblemCommentLike')
...
class ProblemCommentLike(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='leet_problem_comment_likes')
    is_liked = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f'{self.comment.problem.reference}-{self.user.username}'
