...
class BaseMixIn(
    ...
    form_class = lecture_forms.TagForm
    ...
    def get_my_tag_by_lecture(self, lecture):
    ...
    def get_all_tag_list_by_lecture(self, lecture) -> list:
        """Return all_tags corresponding to the targeted problem."""
        if lecture:
            problem_tags = self.tag_model.objects.filter(lecture=lecture)
            tags_list = []
            if problem_tags:
    ...
    def get_tags_from_tag_id_list(tag_id_list):
        content_type = ContentType.objects.get(app_label='lecture', model='tag')
