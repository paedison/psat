...
#     template_name = 'lecture/v1/snippets/comment_list_card.html'
#
...
#         pagination_url = reverse_lazy('lecture:comment_list')
#
#         context.update({
#             'page_obj': page_obj,
...
class ContainerView(
    ...
    """View for loading comment container."""
    template_name = 'lecture/v1/snippets/comment_container.html'

    def get_context_data(self, **kwargs):
        lecture_id = self.kwargs.get('lecture_id')
        comment_qs = utils.get_comment_qs()
        all_comments = utils.get_all_comments(comment_qs, lecture_id)
        page_obj, page_range = self.get_paginator_info(all_comments, per_page=5)
        pagination_url = reverse_lazy('lecture:comment_container', args=[lecture_id])
        reload_url = reverse_lazy('lecture:comment_container', args=[lecture_id])
        create_url = reverse_lazy('lecture:comment_create', args=[lecture_id])
...
#     vanilla.DetailView,
# ):
#     template_name = 'lecture/v1/snippets/comment_detail.html'
#
#     def get_template_names(self):
#         htmx_template = {
...
#         pagination_url = reverse_lazy('lecture:comment_detail_content', args=[parent_comment.id])
#         create_url = reverse_lazy('lecture:comment_create', args=[parent_comment.problem_id])
#
#         return super().get_context_data(
...
class CreateView(
    ...
    template_name = 'lecture/v1/snippets/comment_modal.html#create'

    def get_success_url(self):
        lecture_id = self.kwargs.get('lecture_id')
        return reverse_lazy('lecture:comment_container', args=[lecture_id])
...
class UpdateView(
    ...
    template_name = 'lecture/v1/snippets/comment_modal.html#update'

    def get_success_url(self):
        lecture_id = self.request.POST.get('lecture_id')
        page_number = self.request.POST.get('page', '1')
        url = reverse_lazy('lecture:comment_container', args=[lecture_id])
