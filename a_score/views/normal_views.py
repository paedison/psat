    ...
    if request.user.is_authenticated:
        for obj in page_obj:
            ...
            if student:
                ...
                # obj.detail_url = reverse_lazy('score_prime_psat:detail', args=[obj['year'], obj['round']])
...
def detail_view(request: HtmxHttpRequest, exam_year: int, exam_round: int):
    ...
    qs_answer_official = get_object_or_404(PrimePsatExam, year=exam_year, round=exam_round)
    ...
    data_rate_selection: dict[str, list] = {'heonbeob': [], 'eoneo': [], 'jaryo': [], 'sanghwang': []}
    subject_vars = {'헌법': 'heonbeob', '언어': 'eoneo', '자료': 'jaryo', '상황': 'sanghwang'}
    for qs in qs_answer_count:
        field: str = subject_vars[qs.subject]
        answer_official = qs_answer_official.answer_official[field][qs.number - 1]
...
def no_open_modal_view(request: HtmxHttpRequest):
    context = update_context_data(
    )
    return render(request, 'a_score/snippets/prime_modal.html#no_open_modal', context)


def no_student_modal_view(request: HtmxHttpRequest):
    context = update_context_data(
    )
    return render(request, 'a_score/snippets/prime_modal.html#no_student_modal', context)
...
def student_connect_modal_view(request: HtmxHttpRequest, exam_year: int, exam_round: int):
    ...
    context = update_context_data(header=header, exam_year=exam_year, exam_round=exam_round)
