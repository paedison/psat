...
from a_psat.utils.variables import PsatContext, SubjectVariants
...
def predict_list_view(request: HtmxHttpRequest):
    ...
    context = update_context_data(
        current_time=timezone.now(),
        config=config,
        psats=list_ctx.get_psats_context(),
        login_url=list_ctx.get_login_url_context(),
    )
...
def predict_detail_view(request: HtmxHttpRequest, pk: int, student=None):
    ...
    psat_ctx = PsatContext(_psat=psat)
    subject_variants = SubjectVariants(_psat=psat)
    redirect_ctx = NormalRedirectContext(_request=request, _context=context)

    if not request.user.is_superuser and psat_ctx.is_not_for_predict():  # noqa
        return redirect_ctx.redirect_to_no_predict_psat()
    ...
    context = update_context_data(
        context,
        student=student,
        time_schedule=psat_ctx.get_time_schedule(),
    ...
    detail_ctx = NormalDetailContext(_request=request, _context=context)
    total_statistics_context = detail_ctx.get_normal_statistics_context(False)
    filtered_statistics_context = detail_ctx.get_normal_statistics_context(True)

    chart_ctx = ChartContext(_statistics_context=total_statistics_context, _student=student)
...
def predict_register_view(request: HtmxHttpRequest):
    ...
    # Redirect page
    if psat_ctx.is_not_for_predict():
...
def predict_answer_input_view(request: HtmxHttpRequest, pk: int, subject_field: str):
    context, redirect_ctx, redirect_response = prepare_psat_context(request, pk, subject_field)
...
def prepare_psat_context(request: HtmxHttpRequest, pk: int, subject_field: str):
    ...
    if psat_ctx.is_not_for_predict():
        return None, None, redirect_ctx.redirect_to_no_predict_psat()
    if psat_ctx.before_exam_start():
