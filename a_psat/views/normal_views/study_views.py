...
def study_list_view(request: HtmxHttpRequest):
    ...
    context = update_context_data(config=config, current_time=current_time, students=list_context.get_student_context())
...
def study_detail_view(request: HtmxHttpRequest, pk: int, student=None):
    ...
    detail_context = NormalDetailContext(request=request, student=student)
    context = update_context_data(
        context, current_time=detail_context.current_time,
    ...
    if view_type == 'my_result':
        context = update_context_data(context, my_result_context=detail_context.get_my_result_context())
        return render(request, 'a_psat/snippets/study_detail_my_result.html', context)
    if view_type == 'statistics':
        context = update_context_data(
            context,
            curriculum_stat=detail_context.statistics['total'],
            statistics_context=detail_context.get_statistics_context(),
        )
        return render(request, 'a_psat/snippets/study_detail_statistics.html', context)
    if view_type == 'answer_analysis':
        context = update_context_data(context, answer_context=detail_context.get_answer_context())
    ...
    context = update_context_data(
        ...
        my_result_context=detail_context.get_my_result_context(),
        curriculum_stat=detail_context.statistics['total'],
        statistics_context=detail_context.get_statistics_context(),
        answer_context=detail_context.get_answer_context(),
    )
...
def study_answer_input_view(request: HtmxHttpRequest, pk: int):
    ...
    answer_process_context = NormalAnswerProcessContext(request=request, result=result)

    if request.method == 'POST':
        return answer_process_context.process_post_request_to_submit_answer(context)

    context = update_context_data(
        context, page_title=page_title, schedule=schedule,
        student=result.student, answer_student=answer_process_context.get_answer_student(),
    )
...
def study_answer_confirm_view(request: HtmxHttpRequest, pk: int):
    ...
    if request.method == 'POST':
        answer_process_context = NormalAnswerProcessContext(request=request, result=result)
        answer_process_context.process_post_request_to_confirm_answer()

        next_url = result.student.get_study_curriculum_detail_url()
        context = update_context_data(
            header=f'답안 제출', is_confirmed=answer_process_context.is_confirmed, next_url=next_url)
