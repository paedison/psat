...
from django.db.models import Case, When, Value, BooleanField, Count, Window
...
def modal_view(request: HtmxHttpRequest, pk: int):
    ...
    if view_type == 'student_register':
        units = models.choices.predict_unit_choice()[exam.exam_abbr]
        context = update_context_data(
            context,
            exam_vars=exam_vars,
            units=units,
...
class ExamVars:
    ...
    def get_qs_student_answer(self, student):
        return models.PredictAnswer.objects.filter(
            problem__psat=self.exam.psat, student=student).annotate(
    ...
    def get_qs_answer_count(self):
        return self.answer_count_model.objects.filter(problem__psat=self.exam.psat).annotate(
    ...
    def create_confirmed_answers(self, student, sub, answer_data):
        ...
        for no, ans in enumerate(answer_data, start=1):
            problem = self.problem_model.objects.get(psat=self.exam.psat, subject=sub, number=no)
