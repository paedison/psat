...
class PsatQuerySet(models.QuerySet):
    def predict_psat_active(self):
        return self.filter(predict_psat__is_active=True).select_related('predict_psat').order_by('-id')
...
class ProblemQuerySet(models.QuerySet):
    def base_list(self, problem):
        return self.filter(psat=problem.psat, subject=problem.subject)
    ...
    def filtered_problem_by_psat(self, psat):
        return (
            self.select_related('psat').filter(psat=psat)
...
class ProblemCollectionQuerySet(models.QuerySet):
    ...
    def user_collection_for_modal(self, user, pk):
        item_model = apps.get_model('a_psat', 'ProblemCollectionItem')
        collection_ids = (
