...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if rank == 'top':
            answer_count_rank_model = predict_models.AnswerCountTopRank
            ...
            answer_count_rank_model = predict_models.AnswerCountLowRank
            answer_count_rank_model_name = 'AnswerCountLowRank'
            rank_ratio_min = 0.73
            rank_ratio_max = 1
        ...
        def update_answer_count(sub):
            ...
            target_answers = (
                answer_model.objects.filter(
                    sub=sub,
                    student__exam=exam,
                    student__statistics__rank_ratio_total_psat__gte=rank_ratio_min,
                    student__statistics__rank_ratio_total_psat__lte=rank_ratio_max,
                )
            )
            ...
            for i in range(0, problem_numbers):
                ...
                for target in target_counts:
                    ...
                    result[key] = value
                try:
                    instance = answer_count_rank_model.objects.get(
                        exam=exam,
                        sub=sub,
                        number=i + 1,
                    ...
                    if any(fields_not_match):
                        ...
                        update_count += 1
                except answer_count_rank_model.DoesNotExist:
                    ...
                    create_list.append(answer_count_rank_model(**result))
                    create_count += 1
            try:
                with transaction.atomic():
                    if create_list:
                        answer_count_rank_model.objects.bulk_create(create_list)
                        message = f'Successfully {create_count} {answer_count_rank_model_name} {sub} instances created.'
                    elif update_list:
                        answer_count_rank_model.objects.bulk_update(update_list, answer_count_keys)
                        message = f'Successfully {update_count} {answer_count_rank_model_name} {sub} instances updated.'
                    else:
                        message = f'{answer_count_rank_model_name} {sub} instances already exist.'
