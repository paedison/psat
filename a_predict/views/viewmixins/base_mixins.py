...
from a_predict.utils import get_score_stat_sub, get_score_stat_korean
from a_psat import models as reference_models
...
class PredictExamInfo:
    ...
    ex = '행시'
    round = 0
    answer_official_opened_at = datetime.now(pytz.UTC).strftime('%Y-%m-%d %H:%M')
    # category = 'Prime'
    # year = '2024'
    # ex = '프모'
    # round = 5
...
class BaseMixin:
    # models & forms
    department_model = predict_models.Department

    exam_model = predict_models.Exam
    student_model = predict_models.Student
    answer_model = predict_models.StudentAnswer
    ...
    def get_exam(self):
        return self.exam_model.objects.filter(exam=self.ex).first()
...
class NormalBaseMixin(BaseMixin):
    ...
    def get_sub_title(self):
        if self.exam == '프모':
            return f'제{self.round}회 {self.exam} 성적 예측'
        else:
            return f'{self.year}년 {self.exam} 합격 예측'
