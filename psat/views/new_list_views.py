...
class ListViewSetting:
    ...
    @property
    def base_url(self): return reverse_lazy(f'psat:{self.view_type}_list')

    @property
    def pagination_url(self):
        return (f'{self.base_url}?exam__year={self.year}&exam__ex={self.ex}&exam__sub={self.sub}'
                f'&evaluation__user={self.request.user.id}&evaluation__is_liked={self.is_liked}'
                f'&evaluation__difficulty_rated={self.difficulty_rated}'
                f'&evaluation__is_correct={self.is_correct}&data={self.search_data}')

    @property
    def exam__year(self): return self.request.GET.get('exam__year', '')
    @property
    def year(self): return '' if self.exam__year == '' else int(self.exam__year)
    @property
    def ex(self): return self.request.GET.get('exam__ex', '')
    @property
    def sub(self): return self.request.GET.get('exam__sub', '')

    @property
    def is_liked(self):
        is_liked = self.request.GET.get('evaluation__is_liked', '')
        is_liked_dict = {'True': True, 'False': False}
        return '' if is_liked == '' else is_liked_dict[is_liked]

    @property
    def difficulty_rated(self):
        difficulty_rated = self.request.GET.get('evaluation__difficulty_rated', '')
        return '' if difficulty_rated == '' else int(difficulty_rated)

    @property
    def is_correct(self):
        is_correct = self.request.GET.get('evaluation__is_correct', '')
    ...
    def template_name(self):
        if self.view_type == 'search':
            if self.request.method == 'GET':  # For pagination
                return f'{list_base_template}#content'
            else:  # For search in search bar
                return f'{list_base_template}#list_main'
        else:
            list_main_template = f'{self.list_base_template}#list_main'
            list_content_template = f'{self.list_base_template}#content'
            if self.request.method == 'GET':
    ...
    def queryset(self):
        if self.view_type in ['problem', 'search']:
            ...
            return self.get_filtered_queryset('evaluation__difficulty_rated', self.difficulty_rated)
        elif self.view_type == 'answer':
            return self.get_filtered_queryset('evaluation__is_correct', self.is_correct)

    @property
    def year_list(self): return self.queryset.annotate(
        year_suffix=Cast(Concat(F('exam__year'), Value('년')), CharField())
    ).values_list('exam__year', 'year_suffix')

    @property
    def ex_list(self): return self.queryset.values_list('exam__ex', 'exam__exam2')
    @property
    def sub_list(self): return self.queryset.values_list('exam__sub', 'exam__subject')
    ...
    @property
    def year_option(self): return self.get_option(self.year_list)
    @property
    def ex_option(self): return self.get_option(self.ex_list)
    @property
    def sub_option(self): return self.get_option(self.sub_list)
    @property
    def like_option(self): return [(True, '즐겨찾기 추가 문제'), (False, '즐겨찾기 제외 문제')]
    @property
    def answer_option(self): return [(True, '맞힌 문제'), (False, '틀린 문제')]

    @property
    def rate_option(self): return [
        (1, '★☆☆☆☆'),
        (2, '★★☆☆☆'),
        (3, '★★★☆☆'),
        (4, '★★★★☆'),
    ...
    @property
    def page_obj(self): return self.get_paginator_info()[0]
    @property
    def page_range(self): return self.get_paginator_info()[1]
    ...
    def context(self):
        ...
        info = {
            ...
            'difficulty_rated': self.difficulty_rated,
...
#
#     pagination_url = f'{base_url}?evaluation__User{request.user.id}&exam__sub={sub}&evaluation__is_correct={is_correct}'
#     page_obj, page_range = get_paginator_info(request, 10, queryset)
#
#     info = get_info(
#         view_type, pagination_url=pagination_url, base_url=base_url,
